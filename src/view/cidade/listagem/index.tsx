...
function CidadeListagem(props: ListagemProps) {
    ...
    const configuracoesTabela: ConfiguracoesTabela = {
        ...
        colunas: [
            ...
            { nome: 'Estado', campo: 'Nome', entidadePai: 'Estado', tipo: 'string' },
        ],
    }
    ...
    useEffect(() => {
        setCarregando(true);
        api.get(`/OData/Cidade?$count=true&$expand=Estado&$top=${quantidadeRegistrosPorPagina}&$skip=${quantidadeParaPular}`)
            .then(response => {
                setTimeout(() => {
